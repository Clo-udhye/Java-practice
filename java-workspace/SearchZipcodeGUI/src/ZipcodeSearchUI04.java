...
public class ZipcodeSearchUI04 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ZipcodeSearchUI04 frame = new ZipcodeSearchUI04();
			...
			public void mouseClicked(MouseEvent e) {
				...
				for(ZipcodeTO address : addresses) {
					String strAddress = String.format("[%s] %s %s %s %s %s\n", 
							address.getZipcode(), address.getSido(), address.getGugun(), address.getDong(), address.getRi(), address.getBunji());
